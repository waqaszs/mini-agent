...
/**
 * The dedicated tool the model calls to load a skill's full instructions. Named `Skill` with a
 * `skill` parameter, matching the convention production agents (Claude Code, Command Code) use.
 */
 ...
 * only when the model decides a skill is relevant and calls the `Skill` tool.
 */
export function buildSystemPrompt(skills: Skill[]): string {
  ...
  return [
    ...
    "</available_skills>",
    "",
    `When the user's request matches a skill's description, call the \`${SKILL_TOOL_NAME}\` tool with that skill's exact name as the \`skill\` argument to load its full instructions, then follow them precisely. ` +
...
/**
 * Build the `Skill` tool, typed as the SDK's own `Anthropic.Tool` (no hand-rolled duplicate).
 * Its `skill` parameter is constrained to the EXACT set of discovered skill names (an enum),
 * so the model cannot hallucinate a nonexistent skill.
 */
export function buildSkillTool(skills: Skill[]): Anthropic.Tool {
  return {
